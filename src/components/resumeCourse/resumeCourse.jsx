...
const ResumeCourse = ({ title, subtitle, bulletPoints = [], buttonText, buttonLink, imageSrc }) => {
  return (
    <section className="py-spacing-6 bg-black items-start justify-between text-light  ">
      <div className="w-full px-spacing-2 lg:px-spacing-4 flex flex-col lg:flex-row items-center">
        ...
        >
          {/* Título */}
          <h1 className="text-landing-primary text-size-14 sm:text-size-16 font-bobby font-bold mb-spacing-1 text-center sm:text-left ">
            {title}
          </h1>

          {/* Subtítulo */}
          <h2 className="text-btn-cta-yellow font-bobby text-size-14 sm:text-size-16 font-bold mb-spacing-3 text-left">
            {subtitle}
          </h2>
          ...
          <ul className="text-light list-disc pl-6 mb-spacing-6">
            {bulletPoints.map((point, index) => (
              <li key={index} className="text-light text-size-4 sm:text-size-5 mb-2">
                {point}
              </li>
            ))}
          </ul>
